...
class VPSMgr(object):
    ...
    def run_once(self, cmd, vps_id, vps_info):
        ...
        if callable(h):
            try:
                ...
                self._unlock_vps(cmd, vps_id)
                self.logger.exception("vps %s, uncaught exception: %s" %
                                      (vps_info.id, str(e)))
                self.done_task(cmd, vps_id, False, "uncaught exception %s" %
                               (str(e)))
                return False
        else:
            self.logger.warn("no handler for cmd %s, vps: %s" %
                             (str(cmd), self.dump_vps_info(vps_info)))
    ...
    def run_loop(self, *cmds):
        ...
        while self.running:
            ...
            self.sleep(15)
        self.logger.info("worker for %s stop" %
                         (",".join(map(CMD._get_name, cmds))))
    ...
    def dump_vps_info(vps_info):
        ip = vps_info.ext_ips and "(%s)" % ",".join(
            map(lambda ip: "%s/%s(%s)" % (ip.ipv4, ip.ipv4_netmask, ip.mac), vps_info.ext_ips)) or None
        if vps_info.int_ip and vps_info.int_ip.ipv4:
            ip_inter = "%s/%s" % (vps_info.int_ip.ipv4,
                                  vps_info.int_ip.ipv4_netmask)
        else:
            ip_inter = None
        if vps_info.gateway and vps_info.gateway.ipv4:
            gateway = "%s/%s" % (vps_info.gateway.ipv4,
                                 vps_info.gateway.ipv4_netmask)
        else:
            gateway = None
        hd = vps_info.harddisks and "(%s)" % ",".join(
            map(lambda x: "%s:%s" % (x[0], x[1]), vps_info.harddisks.unwrap().items())) or None
        if vps_info.state is not None:
            state = "%s(%s)" % (vps_info.state,
                                VM_STATE_CN.get(vps_info.state))
        else:
            state = None
    ...
    def setup_vps(self, xenvps, vps_info):
        ...
        if vps_info.ext_ips:
            ...
            xenvps.add_netinf_ext(
                ip_dict, mac=vps_info.ext_ips[0].mac, bandwidth=vps_info.bandwidth)
        ip_inner_dict = dict()
        if vps_info.int_ip and vps_info.int_ip.ipv4:
    ...
    def vps_reinstall_os(self, vps_info):
        self.logger.info("to reinstall vps %s, os=%s" %
                         (vps_info.id, vps_info.os))
        if not self.vpsinfo_check_ip(vps_info):
            msg = "no ip with vps %s" % (vps_info.id)
            self.logger.error(msg)
            ...
            return
        if vps_info.host_id != self.host_id:
            msg = "vps reinstall_os : vps %s host_id=%s != current host %s , abort" % (
                vps_info.id, vps_info.host_id, self.host_id)
            self.logger.error(msg)
            self.done_task(CMD.OS, vps_info.id, False, msg)
            return
    ...
    def _reinstall_os(self, vps_id, vps_info=None, os_id=None, vps_image=None):
        try:
            ...
            self.logger.exception("vps %s: %s" % (str(vps_id), str(e)))
            if vps_info:
                self.done_task(CMD.OS, vps_id, False, "os:%s, error: %s " %
                               (vps_info.os,  str(e)))
            return False
    ...
    def vps_upgrade(self, vps_info):
        ...
        try:
            ...
            self.done_task(CMD.UPGRADE, vps_info.id,
                           False, "exception %s" % str(e))
            return False
    ...
    def vps_reset_pw(self, vps_info):
        ...
        try:
            ...
            self.logger.exception(e)
            self.done_task(CMD.RESET_PW, vps_info.id,
                           False, "exception %s" % (str(e)))
            return
    ...
    def vps_set_bandwidth(self, vps_info):
        ...
        try:
            ...
            self.logger.exception(e)
            self.done_task(CMD.BANDWIDTH, vps_info.id,
                           False, "exception %s" % (str(e)))
            return
    ...
    def _vps_hot_sync(self, vps_id, force=False, to_host_ip=None, speed=None):
        ...
        try:
            ...
            print str(e)
            self.logger.exception(e)
            self.done_task(CMD.PRE_SYNC, vps_id, False, "exception %s" %
                           (str(e)))
            return False
        ...
        try:
            ...
            if task:
                if task.state not in [MIGRATE_STATE.TO_MIGRATE, MIGRATE_STATE.MIGRATING, MIGRATE_STATE.PRE_SYNCED] and not force:
                    raise Exception("task%s state is not TO_MIGRATE" %
                                    (task.id))
                to_host_ip = task.to_host_ip
                ...
                if task.new_ext_ips:
                    ...
                    for ip in task.new_ext_ips:
                        ip_dict[ip.ipv4] = ip.ipv4_netmask
                    xv.add_netinf_ext(
                        ip_dict, mac=task.new_ext_ips[0].mac, bandwidth=task.bandwidth)
                ip_inner_dict = dict()
                if task.new_int_ip and task.new_int_ip.ipv4:
                    ip_inner_dict[
                        task.new_int_ip.ipv4] = task.new_int_ip.ipv4_netmask
            ...
            self.logger.info("to migrate vps%s to host %s" %
                             (vps_id, to_host_ip))
            self.vpsops.migrate_vps(
                migclient, vps_id, to_host_ip, xv=xv, speed=speed)
        except Exception, e:
            print str(e)
            self.logger.exception(e)
            self.done_task(CMD.MIGRATE, vps_id, False, "exception %s" %
                           (str(e)))
            return False
    ...
    def query_migrate_task(self, vps_id):
        ...
        if task.to_host_ip <= 0:
            raise Exception("no destination host ip for task%s vps%s" %
                            (task.id, vps_id))
        return task
    ...
    def refresh_host_space(self):
        ...
        try:
            rpc = self.rpc_connect()
            try:
                rpc.host_refresh(disk_remain, mem_remain,
                                 disk_total, mem_total)
                self.logger.info(
                    "send host remain disk:%dG, mem:%dM, total disk:%dG, mem:%dM" %
                    (disk_remain, mem_remain, disk_total, mem_total))
            finally:
                rpc.close()
        except Exception, e:
            self.logger_net.exception(e)
    ...
    def vps_delete(self, vps_info):
        try:
            ...
            self.done_task(CMD.RM, vps_info.id, False, "exception %s" %
                           (str(e)))
    ...
    def loop(self):
        while self.running:
            # because ovs db searching uses signal and can only work in main
            # thread ...
            self.run_loop(CMD.BANDWIDTH)
            self.sleep(15)
        self.timer.stop()
    ...
    def start_worker(self, *cmds):
        ...
        try:
            ...
            self.logger.info("failed to start worker for cmd %s, %s" %
                             (",".join(map(CMD._get_name, cmds)), str(e)))

    def start(self):
        if self.running:
            return
        self.running = True
        self.start_worker(CMD.OPEN, CMD.CLOSE, CMD.REBOOT,
                          CMD.RM, CMD.RESET_PW)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <= 1:
        ...
        daemon.cmd_wrapper(action, _main, usage, logger,
                           conf.log_dir, conf.RUN_DIR, pid_file, mon_pid_file)
