...
class SSLSocketEngine(TCPSocketEngine):
    ...
    def __init__(self, poll, cert_file, is_blocking=False, debug=False, ssl_version=ssl.PROTOCOL_SSLv23):
        TCPSocketEngine.__init__(
            self, poll, is_blocking=is_blocking, debug=debug)
...
def _do_handshake_server(self, csock, readable_cb, readable_cb_args, idle_timeout_cb, is_blocking=None):
    try:
        ...
        if e.args[0] == ssl.SSL_ERROR_WANT_READ:
            self._poll.register(
                csock.fileno(
                ), 'r', self._do_handshake_server, (csock, readable_cb, readable_cb_args,
                                                    idle_timeout_cb, is_blocking))
        elif e.args[0] == ssl.SSL_ERROR_WANT_WRITE:
            self._poll.register(
                csock.fileno(
                ), 'w', self._do_handshake_server, (csock, readable_cb, readable_cb_args,
                                                    idle_timeout_cb, is_blocking))
        else:
            self.log_error("peer(%s) do_handshake exception %s, %s" %
                           (csock.getpeername(), type(e), str(e)))
            csock.close()
    except(socket.error), e:
        self.log_error("peer(%s) do_handshake exception %s, %s" %
                       (csock.getpeername(), type(e), str(e)))
    ...
    # connection
        try:
            ...
            try:
                ...
                self.log_error(
                    "cannot set FD_CLOEXEC on accepted socket fd, %s" %
                    (str(e)))
            ...
            if callable(new_conn_cb):
                ...
                if not csock:
                    continue
            csock = ssl.wrap_socket(
                csock, certfile=self.cert_file, server_side=True, ssl_version=self.ssl_version,
                do_handshake_on_connect=False)
            self._do_handshake_server(
                csock, readable_cb, readable_cb_args, idle_timeout_cb, is_blocking=is_blocking)
...
def connect_unblock_ssl(self, addr, ok_cb, err_cb=None, cb_args=(), syn_retry=None):
    ...
    def __on_conn(sock, *args):
        ssl_sock = ssl.wrap_socket(
            sock, do_handshake_on_connect=False, suppress_ragged_eofs=False)
        self._do_handshake_client(
            ssl_sock, ok_cb, err_cb, cb_args, stack, is_cb=False)
        return
