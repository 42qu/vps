    ...
    def do(self):
        ...
        try:
            ...
            self.server.logger.info(
                "peer %s, rpc call %s [%s]" % (conn.peer, str(self.req), end_ts - start_ts))
        except Exception, e:
            err = e
            self.server.logger_err.exception(
                "peer %s, rpc call %s failed: %s" % (conn.peer, str(self.req), str(e)))
...
try:
    ...
    class SSL_RPC_Server(object):
        ...
        def __init__(self, cert_file, addr, logger, err_logger=None, timeout=10, idle_timeout=3600, white_list=()):
            ...
            self.engine = SSLSocketEngine(
                io_poll.get_poll(), cert_file=cert_file, is_blocking=True)
            self.engine.set_logger(logger)
            self.engine.set_timeout(
                rw_timeout=timeout, idle_timeout=idle_timeout)
            self.inf_sock = None
            self.addr = addr
        ...
        def start(self, worker_num):
            ...
            self.inf_sock = self.engine.listen_addr_ssl(
                self.addr, readable_cb=self._server_handle, new_conn_cb=self._check_ip)
...
try:
    ...
    class AESInteractJob(InteractJob):
        ...
        def _send(self, resp):
            conn = self.conn
            try:
                NetHead().write_msg(conn.sock,
                                    self.crypter_w.encrypt(resp.serialize()))
                self.server.engine.watch_conn(conn)
            except Exception, e:
                self.server.logger_err.exception(
                    "peer %s, send response error: %s" % (conn.peer, str(e)))
        ...
        def _server_handle(self, conn):
            ...
            try:
                if head.body_len < self.block_size:
                    self.logger.error(
                        "from peer: %s, smaller than block_size head received" % (str(conn.peer)))
                    self.engine.close_conn(conn)
                    return
                buf = head.read_data(sock)
