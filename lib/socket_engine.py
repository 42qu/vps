...
class Connection(object):
    ...
    def __init__(self, sock, readable_cb=None, readable_cb_args=(), idle_timeout_cb=None):
        ...
        self.idle_timeout_cb = callable(
            idle_timeout_cb) and idle_timeout_cb or None
...
#                            res = True
                ...
                self._conn_callback(conn, conn.readable_cb,
                                    (conn, ) + conn.readable_cb_args, count=1)
                return
            try:
                ...
                if conn.is_blocking:
                    self._poll.register(
                        conn.fd, 'r', conn.readable_cb, (conn, ) + conn.readable_cb_args)
                else:
                    self._poll.register(
                        conn.fd, 'r', self._unblock_readable, (conn, ))
            except Exception, e:
                self.logger.error("peer %s: watch conn error %s" %
                                  (conn.peer, str(e)))
        else:
            self._lock()
    ...
    def remove_conn(self, conn):
        ...
        if thread.get_ident() == self._poll_tid:
            ...
            if self._sock_dict.has_key(fd):
                ...
                try:
                    # NOTE: the event is to be reconsider with libev
    ...
    def _accept_conn(self, sock, readable_cb, readable_cb_args, idle_timeout_cb, new_conn_cb, is_blocking=False):
        ...
        while True:
        # have to make sure the socket is non-block, so we can accept multiple
        # connection
            try:
                (csock, addr) = _accept()
                try:
                    ...
                    self.log_error(
                        "cannot set FD_CLOEXEC on accepted socket fd, %s" % (str(e)))
                if is_blocking:
                ...
                if callable(new_conn_cb):
                    ...
                    if not csock:
                        continue
                _put_sock(
                    csock, readable_cb=readable_cb, readable_cb_args=readable_cb_args,
                    idle_timeout_cb=idle_timeout_cb, stack=False)
            except socket.error, e:
                if e[0] == errno.EAGAIN:
                    return  # no more
                if e[0] != errno.EINTR:
                    msg = "accept(sock %s %s) error(unlikely): %s" % (
                        sock, readable_cb, str(e))
                    self.log_error(msg)
        return
    ...
    def listen(self, sock, readable_cb, readable_cb_args=(),
        ...
        try:
            ...
            self.log_error("cannot set FD_CLOEXEC on listening socket fd, %s" %
                           (str(e)))
        sock.listen(backlog)
...
#            raise Exception("not possible")
        ...
        while expect_len:
            try:
                ...
                if not _len:
                    ...
                    break
                # always read more to support epoll ET mode, put the rest into
                # read ahead buffer
                if _len > expect_len:
                ...
                if e[0] in self._eagain_errno:
                    ...
                    self._sock_dict[conn.fd] = conn
                    self._poll.register(
                        conn.fd, 'r', self._do_unblock_read, (conn, ok_cb, ))
        ...
        if conn.error is None:
            self._poll.replace_read(conn.fd, self._read_ahead, (conn, ))
            self._conn_callback(conn, ok_cb, (conn, ) +
                                conn.read_cb_args, stack=conn.read_tb)
        else:
            self._conn_callback(conn, conn.read_err_cb, (conn, )
                                + conn.read_cb_args, stack=conn.read_tb)
        ...
        if pos < 0:
            if len(conn.rd_ahead_buf) > max_len:
                ...
                if not eof:
                    ...
                    self._sock_dict[conn.fd] = conn
                    self._poll.register(
                        conn.fd, 'r', self._do_unblock_readline, (conn, ok_cb, max_len))
                    return False
                else:
                    conn.error = PeerCloseError()
        else:
...
#                    self._lock()
                    self._sock_dict[conn.fd] = conn
                    self._poll.register(
                        conn.fd, 'w', self._do_unblock_write, (conn, buf, ok_cb))
#                    self._unlock()
        ...
        self._conn_callback(
            conn, conn.error is None and ok_cb or conn.write_err_cb,
            (conn, ) + conn.write_cb_args, stack=conn.write_tb)
    ...
    def read_unblock(self, conn, expect_len, ok_cb, err_cb=None, cb_args=()):
        ...
        if self._do_unblock_read(conn, ok_cb, direct=True):
            self._conn_callback(
                conn, conn.error is None and ok_cb or err_cb, (conn, ) + cb_args, count=2)
        else:
    ...
    def _conn_callback(self, conn, cb, args, stack=None, count=1):
        if conn.error is not None:
            # NOTICE: we will close the conn before err_cb
            self.close_conn(conn)
        if not callable(cb):
            return
    ...
    def _exec_callback(self, cb, args, stack=None):
        try:
            ...
            msg = "uncaught %s exception in %s %s:%s" % (
                type(e), str(cb), str(args), str(e))
            if stack:
                l_out = stack
                exc_type, exc_value, exc_traceback = sys.exc_info()
                l_in = traceback.extract_tb(exc_traceback)[1:]  # 0 is here
                stack_trace = "\n".join(
                    map(lambda f: "in '%s':%d %s() '%s'" % f, l_out + l_in))
                msg += "\nprevious stack trace [%s]" % (stack_trace)
                self.log_error(msg)
            else:
                self.log_exception(msg)
    ...
    def poll(self, timeout=100):
        ...
        try:
            ...
            # locking when poll may be prevent other thread to lock, but it's
            # possible poll is not thread-safe, so we do the lazy approach
...
class TCPSocketEngine(SocketEngine):
    ...
    def listen_addr(
        self, addr, readable_cb, readable_cb_args=(), idle_timeout_cb=None,
        ...
        self.bind_addr = addr
        self.listen(
            sock, readable_cb=readable_cb, readable_cb_args=readable_cb_args,
            idle_timeout_cb=idle_timeout_cb,
    ...
    def connect_unblock(self, addr, ok_cb, err_cb=None, cb_args=(), syn_retry=None):
        ...
        try:
            ...
            try:
                ...
                self.log_error(
                    "cannot set FD_CLOEXEC on connecting socket fd, %s" % (str(e)))
        # when addr resolve error, connect_ex will raise exception
        except socket.error, e:
            sock.close()
            if callable(err_cb):
        ...
        def __on_connected(sock):
            ...
            if res_code == 0:
                ...
                if callable(err_cb):
                    self._exec_callback(
                        err_cb, (ConnectNonblockError(res_code, err_msg),) + cb_args, stack)
            return
        if res == 0:
            self._exec_callback(ok_cb, (sock, ) + cb_args, stack)
            ...
            if callable(err_cb):
                self._exec_callback(
                    err_cb, (ConnectNonblockError(res, err_msg), ) + cb_args, stack)
            return False
