...
def _check_disk(client, vps_id, dry=True):
    ...
    if not os.path.exists(meta):
        ...
        if not os.path.exists(meta):
            meta = client.vpsops._meta_path(
                vps_id, is_trash=True, is_deleted=True)
            if not os.path.exists(meta):
                meta = client.vpsops._meta_path(
                    vps_id, is_trash=False, is_deleted=True)
                if not os.path.exists(meta):
                    return
            is_delete = True
        else:
    ...
    if is_delete:
        ...
        if not vps_info:
            if is_trash:
                _delete_all(logger, xv, "closed but no info",
                            dry=dry, days=7, metapath=meta)
            else:
                _delete_trash(logger, xv, "running but no info",
                              dry=dry, days=7)
        elif vps_info.state == VM_STATE.CLOSE:
            if str(vps_info.host_id) != str(conf.HOST_ID):
                _delete_all(logger, xv, "closed but not on this host",
                            dry=dry, days=7, metapath=meta)
            else:
            ...
            if str(vps_info.host_id) != str(conf.HOST_ID):
                _delete_all(logger, xv, "running but not on this host",
                            dry=dry, days=7, metapath=meta)
            else:
