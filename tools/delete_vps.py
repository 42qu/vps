...
def delete_vps (vps_id, forced=False):
    ...
    if not vps:
        ...
        if not forced:
            return
    if vps and vps.host_id == conf.HOST_ID and vps.state != vps_const.VPS_STATE.RM: 
        print "vps %s state=%s, is not to be deleted" % (vps_id, vps_const.VPS_STATE_CN[vps.state])
        if not forced:
            return
